// Occasional Idiot
// Source file for Chrome plugin to handle G+ integration.
// G+ DOM manipulation routines.
...
var processWallPost = function(postNode) {
  try {
    ...
    var postername;
    try {
      postername = postNode.getElementsByClassName(GPLUS_PERSON_LINK_CLASSNAME)[0].
          innerText.trim();
    } catch(ex) {
    }
...
var processComment = function(commentNode) {
  try {
    var commenter = commentNode.getElementsByClassName(GPLUS_PERSON_LINK_CLASSNAME)[0].
        innerText.trim();
...
var processAllChildPostNodes = function(node) {
  if (node.classList && node.classList.contains(GPLUS_POST_DIV_CLASSNAME)) {
    processWallPost(node);
  } else {
    var postNodes = node.getElementsByClassName(GPLUS_POST_DIV_CLASSNAME);
...
var processAllChildCommentNodes = function(node) {
  if (node.classList && node.classList.contains(GPLUS_COMMENT_DIV_CLASSNAME)) {
    processComment(node);
  } else {
    var commentNodes = node.getElementsByClassName(GPLUS_COMMENT_DIV_CLASSNAME);
